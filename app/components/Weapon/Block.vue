...
const { data } = defineProps<{ data: WeaponData }>();

function getRarityBgColor(rarity: number) {
  if (rarity) {
    switch (rarity) {
      case 1:
...
function getRarityRingColor(rarity: number) {
  if (rarity) {
    switch (rarity) {
      case 1:
        return "ring-#868586";
...
<template>
  <NuxtLink
    ...
    :class="getRarityRingColor(data.rarity)"
  >
    <div class="relative">
      <NuxtImg
        ...
        :class="getRarityBgColor(data.rarity)"
        placeholder
      />
    </div>
